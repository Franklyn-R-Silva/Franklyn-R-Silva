...
- **Live streak stats**: `fetch_streak()` scrapes the three headline numbers (total contributions / current / longest streak) from the `github-readme-streak-stats` SVG at build time and bakes them into the "Live Stats" section (`sec2` line + two `Grid.*` lines). It returns `None` on any network/parse failure, and the `LINES` tail falls back to the static "stats below ↓" line so the build never breaks offline or during a CI hiccup. The daily `refresh-card` job keeps the numbers current.
